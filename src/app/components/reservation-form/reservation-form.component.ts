...
export class ReservationFormComponent {
    ...
    tomorrow: string = new Date(new Date().setDate(new Date().getDate() + 1))
        .toISOString()
        .split("T")[0];
    ...
    private initializeForm(): void {
        this.reservationForm = new FormGroup({
            ...
            phone: new FormControl(this.reservation?.phone || "", [
                Validators.required,
                Validators.pattern(
                    /^(?:\+(\d{2}))?\s?(\(?([1-9]{2}|[0-9]{3})\)?)\s?(\d{4,5})-?(\d{4})$/
                ),
            ]),
            date: new FormControl(this.reservation?.date || "", [
                Validators.required,
                ValidateDate,
            ]),
            time: new FormControl(this.reservation?.time || "select", [
                Validators.required,
                Validators.pattern(
                    /^(19:00|19:30|20:00|20:30|21:00|21:30|22:00|22:30|23:00)$/
                ),
            ]),
            peopleNumber: new FormControl(this.reservation?.peopleNumber || "", [
                Validators.required,
